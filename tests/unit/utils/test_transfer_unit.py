...
class PingEndpointTests:
    ...
    async def test_ping_endpoint_http_error_max_retries(self):
        ...
        with patch(
            ...
            mock_session_class.return_value = mock_session_ctx
            with patch('asyncio.sleep', new_callable=AsyncMock):
                with raises(ConnectionError, match='ping failed after 2 attempts'):
                    await specimen.ping_endpoint(
                        'https://host', 'token', max_retries=2
                    )

    @mark.asyncio
    async def test_ping_endpoint_connector_error_dns_retries_then_success(self):
        call_count = [0]
    ...
    @mark.asyncio
    async def test_ping_endpoint_connector_error_backoff_after_first_retry(self):
        call_count = [0]
...
class UploadChunkTests:
    ...
    async def test_upload_chunk_success(self):
        ...
        with patch("trainml.utils.transfer.retry_request", new_callable=AsyncMock) as mock_retry_patch:
    ...
    async def test_upload_chunk_error_status(self):
        ...
        with patch("trainml.utils.transfer.retry_request") as mock_retry:
            mock_retry.side_effect = ConnectionError("Chunk 0-3 failed with status 400: Bad Request")
                ...
                with patch("os.path.isfile", return_value=False):
                    with patch("os.path.isdir", return_value=False):
                        with patch("os.path.exists", return_value=True):
                            with raises(ValueError, match="Path is neither a file nor directory"):
                ...
                with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                    ...
                    with patch("aiohttp.ClientSession") as mock_session:
                        ...
                        with patch(
                            ...
                            mock_post_context = AwaitableContextManager(mock_finalize_response)
                ...
                with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                    ...
                    with patch("aiohttp.ClientSession") as mock_session:
                        ...
                        with patch(
                            ...
                            await specimen.upload("example.com", "token", tmpdir)
                            # Verify upload_chunk was called
                            assert mock_upload_chunk.called
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                ...
                with patch("aiohttp.ClientSession") as mock_session:
                    ...
                    with patch("trainml.utils.transfer.ping_endpoint", new_callable=AsyncMock):
                        with patch("trainml.utils.transfer.upload_chunk", new_callable=AsyncMock):
                            with raises(TrainMLException, match="tar command failed"):
                                await specimen.upload("example.com", "token", tmp.name)
    ...
    async def test_upload_multiple_chunks(self):
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                ...
                mock_process.stdout.read = AsyncMock(
                    side_effect=[b"x" * (5 * 1024 * 1024), b"x" * (5 * 1024 * 1024), b""]
                )
                mock_process.returncode = 0
                mock_process.wait = AsyncMock(return_value=0)
                mock_process.stderr.read = AsyncMock(return_value=b"")
...
class DownloadTests:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                with patch(
                    ...
                    with patch("aiofiles.open", return_value=mock_file_context) as mock_file:
                        mock_finalize_response = AsyncMock()
                        mock_finalize_response.status = 200
                        mock_finalize_response.json = AsyncMock(
                            return_value={"status": "ok"}
            ...
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                def mock_get(*args, **kwargs):
                    ...
                    # Handle /info endpoint - raises ConnectionError
                        ...
                        raise ConnectionError("Failed to get server info (status 404): Not found")
                    else:
                        # For /download endpoint, return awaitable that resolves to response
                        async def get_download_response():
                            mock_resp = AsyncMock()
                ...
                with patch(
                    ...
                    side_effect=mock_retry,
                        ...
                        with patch(
                            ...
                            if not hasattr(mock_process, 'stdin') or mock_process.stdin is None:
                                mock_process.stdin = Mock()
                                mock_process.stdin.write = Mock()
                                mock_process.stdin.drain = AsyncMock()
                                mock_process.stdin.close = Mock()
    ...
    async def test_download_info_endpoint_non_404_error(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                with patch(
                    "trainml.utils.transfer.retry_request",
                    side_effect=ConnectionError("Failed to get server info (status 500): Server error"),
                ):
                    with patch("trainml.utils.transfer.ping_endpoint", new_callable=AsyncMock):
                ...
                async def _get_info_with_error(*args, **kwargs):
                    ...
                    async with mock_resp:
                        if mock_resp.status != 200:
                            ...
                            raise ConnectionError(f"Failed to get server info (status {mock_resp.status}): {error_text}")
            ...
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                with patch(
                    ...
                    with patch("aiofiles.open", return_value=mock_file_context) as mock_file:
                        with patch("trainml.utils.transfer.ping_endpoint", new_callable=AsyncMock):
                            with raises(
                                ConnectionError, match="Downloaded file is empty"
                            ):
                                await specimen.download(
                                    "example.com", "token", tmpdir
                                )
    ...
    async def test_download_non_404_error_in_download(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                async def _download(*args, **kwargs):
                    ...
                    if mock_resp.status != 200:
                        ...
                        if mock_resp.status == 404:
                            raise ConnectionError("Download endpoint not available (404)")
                        raise ClientResponseError(
                ...
                with patch(
                    "trainml.utils.transfer.retry_request",
                    side_effect=[{"archive": False}, ClientResponseError(
                        request_info=Mock(),
                        history=(),
                        status=500,
                        message="Server error",
    ...
    @mark.asyncio
    async def test_download_content_disposition_filename_no_quotes_fallback(self):
    ...
    async def test_download_finalize_success_logging(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                mock_info_response.__aenter__ = AsyncMock(return_value=mock_info_response)
                ...
                def mock_get(*args, **kwargs):
                    ...
                    if call_count == 1:
                        ...
                        mock_get_response.__aenter__ = AsyncMock(return_value=mock_info_response)
    ...
    async def test_download_info_endpoint_error_reading_body_direct(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                mock_info_response = AsyncMock()
                mock_info_response.status = 500
                mock_info_response.text = AsyncMock(side_effect=Exception("Read error"))
                mock_info_response.request_info = Mock()
                mock_info_response.history = ()
                mock_info_response.__aenter__ = AsyncMock(return_value=mock_info_response)
                mock_info_response.__aexit__ = AsyncMock(return_value=None)
                ...
                with patch("trainml.utils.transfer.retry_request", side_effect=mock_retry):
                    with patch("trainml.utils.transfer.ping_endpoint", new_callable=AsyncMock):
                        with raises(ConnectionError, match="Failed to get server info.*Unable to read response body"):
                            await specimen.download("example.com", "token", tmpdir)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                async def chunk_iter():
                    yield b"zip data"
                    yield b""
                mock_download_response.content.iter_chunked = lambda size: chunk_iter()
                mock_download_response.close = Mock()
                mock_download_response.request_info = Mock()
                mock_download_response.history = ()
                ...
                with patch("trainml.utils.transfer.retry_request", side_effect=mock_retry):
                    with patch("trainml.utils.transfer.ping_endpoint", new_callable=AsyncMock):
                        ...
                        def mock_re_search(pattern, string, *args, **kwargs):
                            ...
                            if search_call_count == 1 and 'filename="?([^"]+)"?' in pattern:
                                return None
                        ...
                        mock_finalize_response.__aenter__ = AsyncMock(return_value=mock_finalize_response)
                        ...
                        with patch("trainml.utils.transfer.retry_request", side_effect=mock_retry):
                            with patch("trainml.utils.transfer.re.search", side_effect=mock_re_search):
                                with patch("aiofiles.open", return_value=mock_file_context):
                                    await specimen.download("example.com", "token", tmpdir)
                                    # Verify the file was written (filename parsing worked)
                                    assert mock_file_context.__aenter__.return_value.write.called

...
_TRAINML_QUERY_TEST_ENV: Dict[str, str] = {
...
async def test_trainml_query_success(mock_open, mock_requests_get, mock_boto3_client):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        # Mock the auth config request
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "region": "us-east-1",
...
async def test_trainml_query_auth_error(mock_open, mock_requests_get, mock_boto3_client):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        trainml.auth.get_tokens = MagicMock(
            side_effect=TrainMLException("Auth failed")
        )
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_generic_auth_error(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        with raises(TrainMLException) as exc_info:
            await trainml._query("/test", "GET")
        assert "Error getting authorization tokens" in str(
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_with_headers(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        trainml = specimen.TrainML()
        trainml.auth.get_tokens = MagicMock(
            return_value={"id_token": "token123"}
        )

        mock_resp = create_mock_aiohttp_response(
            json_data={"result": "success"}
        )
        mock_session_ctx, mock_session = create_mock_aiohttp_session(
            [mock_resp]
        )

        with patch(
            "trainml.trainml.aiohttp.ClientSession",
            return_value=mock_session_ctx,
        ):
            result = await trainml._query(
                "/test", "GET", headers={"X-Custom": "value"}
            )
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_params_validation(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        assert "Query parameters must be a valid dictionary" in str(
            exc_info.value.message
        )
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_boolean_params(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        with patch(
            ...
            await trainml._query(
                "/test", "GET", params={"flag": True, "other": False}
            )
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_project_uuid_injection(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_502_retry(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        mock_resp_502 = create_mock_aiohttp_response(
            status=502, read_data=b'{"error": "Bad Gateway"}'
        )
        mock_resp_success = create_mock_aiohttp_response(
            json_data={"result": "success"}
        )
        mock_session_ctx, mock_session = create_mock_aiohttp_session(
            [mock_resp_502, mock_resp_success]
        )
        ...
        with patch(
            ...
            with patch(
                "trainml.trainml.asyncio.sleep", new_callable=AsyncMock
            ):
                result = await trainml._query("/test", "GET")
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_json_error_response(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_non_json_error_response(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_client_response_error(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _TRAINML_QUERY_TEST_ENV):
        ...
        # The code raises ApiError with a string, which causes an
        # AttributeError. This is actually a bug in the code, but we test
        # that it raises an error.
        with patch(
            "trainml.trainml.aiohttp.ClientSession",
            return_value=mock_session_ctx,
        ):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_trainml_query_max_retries_exceeded(
    mock_open, mock_requests_get, mock_boto3_client
):
